...
def set_mpmath_precision(dps=20):
    mpm.mp.dps=dps
def phi_func_val_at_x(x, reconstruction_order, jump_loc, jump_mag):
    ...
    for l in range(d + 1):
        ...
        s = mpm.fadd(s, s1)
    return s
def psi_func_val_at_x(x, reconstruction_order, func_coeff, jump_loc, jump_mag):
    ...
    for k in range(-m, m + 1):
        phi_k_coeff = __calc_coeff_phi(k, reconstruction_order=reconstruction_order, jump_loc=jump_loc,
                                       jump_mag=jump_mag)
        psi_k_coeff = __calc_coeff_psi(func_coeff_at_k=func_coeff[0,m+k], phi_coeff_at_k=phi_k_coeff)
        s = mpm.fadd(s, mpm.fmul(psi_k_coeff, mpm.expj(mpm.fmul(k, x))))
    return s
def func_val_at_x(x, reconstruction_order, func_coeff, jump_loc, jump_mag):
    phi_val = phi_func_val_at_x(x=x, reconstruction_order=reconstruction_order, jump_loc=jump_loc,
                                jump_mag=jump_mag)
    psi_val = psi_func_val_at_x(x=x, reconstruction_order=reconstruction_order, func_coeff=func_coeff,
...
def __vn_func_val_at_x(x, n, jump_loc):
    ...
    return s
def __calc_coeff_phi(k, reconstruction_order, jump_loc, jump_mag):
    if k==0:
        return 0
    else:
